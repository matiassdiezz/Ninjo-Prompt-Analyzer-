...
import { applyChange, applyAllChanges } from '@/lib/utils/changeApplier';
...
export function NinjoChatPanel() {
  const { currentPrompt, chatMessages, addChatMessage, setPrompt, pushUndo, undo, updateMessageChangeStatus } = useAnalysisStore();
  ...
  const handleApplyChange = useCallback((change: ParsedChange) => {
    try {
      pushUndo();
      const result = applyChange(currentPrompt, change);
      if (result.success) {
  ...
  const handleApplyAllChanges = useCallback((changes: ParsedChange[]) => {
    try {
      pushUndo();
      const { newPrompt, appliedCount, failedCount } = applyAllChanges(currentPrompt, changes);
      setPrompt(newPrompt);
  ...
  // Handle undo a previously applied change
  const handleUndoChange = useCallback(() => {
    undo();
    addToast('Cambio deshecho', 'info');
  }, [undo, addToast]);

  // Handle persisting change status to store
  const handleChangeStatusUpdate = useCallback((messageId: string, changeIndex: number, status: 'applied' | 'rejected' | 'pending') => {
                    ...
                    >
                      {message.role === 'user' ? (
                        ...
                        <MarkdownMessage
                          ...
                          persistedChangeStatuses={message.changeStatuses}
